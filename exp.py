...
import cv2
import torch
from utils import compute_walk_point

def viz_walk_points(ground_truth_route, predicted_route, gt_walk_points, pred_walk_points):
    # Parameters
    scale_x = 200  # horizontal scale
    scale_y = scale_x * 1 / 20  # vertical scale (for 16:9 aspect ratio)
    padding = 20  # for image border

    # --- 1. Find the middle point of ground truth route as the origin ---
    mid_index = len(ground_truth_route) // 2
    origin = (0, 0)  # (x, y)
    
    # --- 2. Compute maximum distance from origin for canvas size ---
    all_points = np.vstack((ground_truth_route, predicted_route))
    max_dx = np.max(np.abs(all_points[:,0] - origin[0]))
    max_dy = np.max(np.abs(all_points[:,1] - origin[1]))
    
    img_width = int(max_dx * 2 * scale_x + 2 * padding)
    img_height = int(max_dy * 2 * scale_y + 2 * padding)
    
    # --- 3. Create a white image ---
    img = np.ones((img_height, img_width, 3), dtype=np.uint8) * 255
    
    # --- 4. Coordinate conversion function ---
    def to_img_coords(pt):
        """
        Convert world coordinates (pt[0], pt[1]) to image coordinates
        with the origin at the image center
        """
        x = int((pt[0] - origin[0]) * scale_x + img_width // 2)
        y = img_height // 2 - int((pt[1] - origin[1]) * scale_y)  # flip y-axis
        return (x, y)
    
    # Draw GT and prediction routes
    for i in range(len(ground_truth_route) - 1):
        p1 = to_img_coords(ground_truth_route[i])
        p2 = to_img_coords(ground_truth_route[i + 1])
        cv2.line(img, p1, p2, (0, 0, 255), 2)  # Red line
    
    for i in range(len(predicted_route) - 1):
        p1 = to_img_coords(predicted_route[i])
        p2 = to_img_coords(predicted_route[i + 1])
        cv2.line(img, p1, p2, (255, 0, 0), 2)  # Blue line
    
    # Draw walk points and correspondence lines
    for gt_pt, pred_pt in zip(gt_walk_points, pred_walk_points):
        gt_px = to_img_coords(gt_pt)
        pred_px = to_img_coords(pred_pt)
        cv2.circle(img, gt_px, 4, (0, 0, 255), -1)   # Red point
        cv2.circle(img, pred_px, 4, (255, 0, 0), -1) # Blue point
        cv2.line(img, gt_px, pred_px, (150, 150, 150), 1)  # Gray connection
...
def clip_veh_route(veh_route, walk_dist=60):
    ...
    # Ensure the vehicle route has standard starting point and final point
    # If matching index equals to the index of last point, return only last point
    matching_index = 0
    ...
    if matching_index == len(veh_route)-1: # The whole route is behind, return empty
        return torch.empty((0, veh_route.shape[1]))

    # Compute start point: y = gps[1]
    ...
    start_point = torch.tensor([x, y])

    # Compute end point
    remain_route = veh_route[matching_index + 1:].clone()

    # Add start point
    subroute = torch.vstack([start_point.unsqueeze(0), remain_route])
...
# ------------------------
# 2. Hit Rate 與 Average Euclidean Error
# ------------------------
def compute_hit_rates(predicted_route, ground_truth_route, threshold):
    # Trace hits along ground truth route: if the distances are far than the distance of ground truth route, then we use -1 to indicate
    # that the interval cannot be counted.
    # Reason: we assume that all test cases are road images but in some area there are no ground truth route.
    intervals = [[0, 10], [10, 20], [20, 30], [30, 40]]
    hits = [0 for intv in intervals]
    num_points = [0 for intv in intervals]
    
    dist = 4 # Initalization distance
    step = 2 # in meters
    gt_walk_points = []
    pred_walk_points = []

    while True:
        gt_point, gt_idx = compute_walk_point(torch.tensor(ground_truth_route), dist)
        pred_point = torch.tensor([0, 0])
        if len(predicted_route) > 0:
            pred_point, pred_idx = compute_walk_point(torch.tensor(predicted_route), dist)
        if gt_idx == len(ground_truth_route)-1: # Exceed the route, leave..
            break

        for intv_idx, intv in enumerate(intervals):
            if intv[0] <= dist < intv[1]:
                num_points[intv_idx] += 1
                error = np.linalg.norm(np.array(gt_point) - np.array(pred_point))
                if error < threshold:
                    hits[intv_idx] += 1
        dist += step

        gt_walk_points.append(gt_point.tolist())
        pred_walk_points.append(pred_point.tolist())

    # Normalize hit rates.
    hit_rates = [-1 if curr_num_points == 0 else curr_hits / curr_num_points for curr_hits, curr_num_points in zip(hits, num_points)]
    
    # Compute average hit rate
    total_hits = 0
    total_num_points = 0
    for curr_hits, curr_num_points in zip(hits, num_points):
        if curr_num_points > 0:
            total_hits += curr_hits
            total_num_points += curr_num_points

    hit_rates.append(-1 if total_num_points == 0 else total_hits / total_num_points)

    return hit_rates

def compute_average_euclidean_error(predicted_route, ground_truth_route, missing_penalty=2.0):
    intervals = [[0, 10], [10, 20], [20, 30], [30, 40]]
    errors = [0 for intv in intervals]
    num_points = [0 for intv in intervals]
    ...
    while True:
        ...
        if len(predicted_route) > 0:
            pred_point, pred_idx = compute_walk_point(torch.tensor(predicted_route), dist)
        if gt_idx == len(ground_truth_route)-1: # Exceed the ground truth route, leave..
            break
    ...
    '''
    # DEBUG: Viz. walk points
    walk_img = viz_walk_points(ground_truth_route, predicted_route, gt_walk_points, pred_walk_points)
    cv2.imwrite('output/walk_points.png', walk_img)
    '''
    
    # Normalize errors.
    avg_errors = [-1 if curr_num_points == 0 else curr_error / curr_num_points for curr_error, curr_num_points in zip(errors, num_points)]
    
    # Compute total average error
    total_error = 0
    total_num_points = 0
    for curr_error, curr_num_points in zip(errors, num_points):
        if curr_num_points > 0:
            total_error += curr_error
            total_num_points += curr_num_points

    avg_errors.append(-1 if total_num_points == 0 else total_error / total_num_points)
...
# 3. Jerk 計算
# ------------------------
def compute_jerk(route_points):
    if len(route_points) < 4:
        return []
    vels = [np.array(route_points[i+1]) - np.array(route_points[i])
            for i in range(len(route_points)-1)]
    accs = [vels[i+1] - vels[i] for i in range(len(vels)-1)]
    jerks = [np.linalg.norm((accs[i+1] - accs[i]))
             for i in range(len(accs)-1)]
    return jerks

# ------------------------
# 4. Lateral Acceleration 計算
# ------------------------
def compute_lateral_acceleration(route_points):
    ...
    for i in range(len(accs)):
        ...
        if norm == 0:
            ...
            cross = v[0]*accs[i][1] - v[1]*accs[i][0]
            lat.append(abs(cross)/norm)
    return lat

def compute_metrics(ground_truth_route, predicted_route):
    hit_rates_strict = compute_hit_rates(predicted_route, ground_truth_route, threshold=0.5)
    hit_rates_moderate = compute_hit_rates(predicted_route, ground_truth_route, threshold=1.0)
    hit_rates_loose = compute_hit_rates(predicted_route, ground_truth_route, threshold=2.0)
    euclidean_errors = compute_average_euclidean_error(predicted_route, ground_truth_route)

    metrics = hit_rates_strict
    metrics.extend(hit_rates_moderate)
    metrics.extend(hit_rates_loose)
    metrics.extend(euclidean_errors)

    return metrics

def get_metric_names():
    return ['hit_rate@0.5_0_10', 
            'hit_rate@0.5_10_20', 
            'hit_rate@0.5_20_30', 
            'hit_rate@0.5_30_40', 
            'hit_rate@0.5', 
            ...
            'hit_rate@2.0_30_40', 
            'hit_rate@2.0',
            'euclidean_error_0_10',
            'euclidean_error_10_20',
            'euclidean_error_20_30',
            'euclidean_error_30_40',
            'euclidean_error'] 

def get_exclusive_list():
    no_osm = ['0916', '0035', '0107', '0922', '0109', '0555', '0907', '0520', '0331', '0098', '0096', '0521', '0778', '0269', '0800', '0627', '0626', '0519', '1061', '0038', '0003', '0092', '0796', '0278', '1060', '0018', '0523', '0802', '0522']

    roundabout = ['0523']

    # Temp recorded turn on start here, observe these cases in the future
    turn_on_start = ['0778', '0915', '0102', '0330', '0565', '0638', '0558', '0971', '0276', '0345', '0106', ]
    varied_slope = ['0909', '0920', '0930', '0931', '0972', '0555', '0919'] 
    exclusive_list = no_osm
    exclusive_list.extend(varied_slope)
    exclusive_list = list(set(exclusive_list))
